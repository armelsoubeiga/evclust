...
@pytest.mark.parametrize(
    "x, c, type, alpha, beta, delta, epsi, maxit, disp, gamma, eta, expected_clusters",
    [
...
def test_ecmdd(x, c, type, alpha, beta, delta, epsi, maxit, disp, gamma, eta, expected_clusters):
    # Call the ecm function with the provided inputs
    ecmdd_model = ecmdd(x, c, type=type, alpha=alpha, beta=beta, delta=delta, epsi=epsi, maxit=maxit, disp=disp)

   
    # Check the output elements
    expected_params=['F', 'mass','pl', 'y_pl', 'Y', 'N', 'g', 'D', 'method','W', 'J', 'param']
    for param in expected_params:
        assert param in ecmdd_model
        
    # Check the output type and shape
    assert isinstance(ecmdd_model, dict)
